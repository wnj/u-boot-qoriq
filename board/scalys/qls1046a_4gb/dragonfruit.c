...
int scalys_carrier_init_pcie()
{
	...
	/* Set SLOT4_REFCLK_OE_N to low */
	i2c_set_bus_num(0);
	i2c_read(MOD_I2C_IO_ADDR, 0x0D, 1, &i2c_data, 1);
	i2c_data &= ~SLOT4_REFCLK_OE_N;
	...
	/* Set PEX_PERST_N and PEX_REFCLK_SEL to high */
	val = in_be32(&pgpio2->gpdir) | PEX_PERST_N | PEX_REFCLK_SEL;
	out_be32(&pgpio2->gpdir, val);
	val = in_be32(&pgpio2->gpdat) | PEX_PERST_N;
	val &= ~PEX_REFCLK_SEL;
...
int scalys_carrier_setup_muxing()
{
	 ...
	 */
	val = in_be32(&pgpio2->gpdir) | MUX_SER0_1_SEL | MUX_SER2_3_SEL | MUX_SER5_6_SEL;
	out_be32(&pgpio2->gpdir, val);
	val = in_be32(&pgpio2->gpdat) | MUX_SER0_1_SEL | MUX_SER2_3_SEL;
	val &= ~MUX_SER5_6_SEL;
	out_be32(&pgpio2->gpdat, val);
